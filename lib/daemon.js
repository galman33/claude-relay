...
// --- App name change handler (from client WebSocket) ---
relay.onAppNameChange = function (name) {
  var newAppName = name || null;
  if (newAppName) {
    config.appName = newAppName;
  } else {
    delete config.appName;
  }
  relay.setAppName(newAppName);
  saveConfig(config);
  console.log("[daemon] App name changed via client:", newAppName || "(default: Claude Relay)");
};
...
var ipc = createIPCServer(socketPath(), function (msg) {
  switch (msg.cmd) {
    ...
    case "set_app_name": {
      var newAppName = msg.appName || null;
      if (newAppName) {
        config.appName = newAppName;
      } else {
        delete config.appName;
      ...
      relay.setAppName(newAppName);
      saveConfig(config);
      console.log("[daemon] App name:", newAppName || "(default: Claude Relay)");
      return { ok: true };
    }
