...
var WT_ADJECTIVES = ["bright","calm","cool","dark","fast","glad","keen","mild","neat","warm","bold","fair","kind","pure","soft","wise","wild","deep","true","free"];
var WT_NOUNS = ["fox","oak","elm","bay","sky","sun","owl","jay","ant","bee","ash","dew","gem","ivy","fir","cod","hen","ram","yak","eel"];

function generateWorktreeName() {
  var adj = WT_ADJECTIVES[Math.floor(Math.random() * WT_ADJECTIVES.length)];
  var noun = WT_NOUNS[Math.floor(Math.random() * WT_NOUNS.length)];
...
function sanitizeWorktreeName(name) {
  if (!name) return "";
  return name.trim().toLowerCase().replace(/[^a-z0-9_-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "").substring(0, 50);
}
...
function createProjectContext(opts) {
  ...
  function handleConnection(ws) {
    ...
    if (active) {
      sendTo(ws, { type: "session_switched", id: active.localId, cliSessionId: active.cliSessionId || null, worktree: active.worktreeName || null });
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "new_session") {
      if (msg.worktree) {
        var wtName = sanitizeWorktreeName(msg.worktreeName) || generateWorktreeName();
        var wtPath = path.join(cwd, ".claude", "worktrees", wtName);
        var wtBranch = "worktree-" + wtName;
        try {
          fs.mkdirSync(path.join(cwd, ".claude", "worktrees"), { recursive: true });
          execFileSync("git", ["worktree", "add", wtPath, "-b", wtBranch], { cwd: cwd, timeout: 15000 });
          sm.createSession({ worktreeName: wtName, worktreeCwd: wtPath, title: wtName });
          send({ type: "info", text: "Worktree session created: " + wtName + " (branch: " + wtBranch + ")" });
        } catch (e) {
          send({ type: "error", text: "Failed to create worktree: " + e.message });
        }
      } else {
        sm.createSession();
      }
    ...
    if (msg.type === "delete_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        var sessionToDelete = sm.sessions.get(msg.id);
        var wtCwd = sessionToDelete.worktreeCwd;
        var wtName = sessionToDelete.worktreeName;
        sm.deleteSession(msg.id);
        if (wtCwd) {
          try {
            execFileSync("git", ["worktree", "remove", wtCwd, "--force"], { cwd: cwd, timeout: 15000 });
          } catch (e) {}
          // Also try to delete the branch
          if (wtName) {
            try {
              execFileSync("git", ["branch", "-D", "worktree-" + wtName], { cwd: cwd, timeout: 5000 });
            } catch (e) {}
          }
        }
      }
    ...
    if (msg.type === "rewind_preview") {
      ...
      (async function () {
        var result;
        var rewindCwd = session.worktreeCwd || cwd;
        try {
          ...
          for (var f = 0; f < changedFiles.length; f++) {
            try {
              diffs[changedFiles[f]] = execFileSync(
                ...
                { cwd: rewindCwd, encoding: "utf8", timeout: 5000 }
              ) || "";
