...
export function initNotifications(_ctx) {
  ...
  (function () {
    ...
    // --- App name setting ---
    var appNameBtn = $("footer-app-name-btn");
    var appNameModal = $("app-name-modal");
    var appNameInput = $("app-name-input");
    var appNameSave = $("app-name-save");
    var appNameCancel = $("app-name-cancel");

    if (appNameBtn && appNameModal) {
      appNameBtn.addEventListener("click", function (e) {
        e.stopPropagation();
        footerMenu.classList.add("hidden");
        var current = $("footer-app-name");
        appNameInput.value = current ? current.textContent : "Claude Relay";
        appNameModal.classList.remove("hidden");
        appNameInput.focus();
        appNameInput.select();
      });

      appNameSave.addEventListener("click", function () {
        var val = appNameInput.value.trim();
        if (ctx.ws && ctx.connected) {
          ctx.ws.send(JSON.stringify({ type: "set_app_name", appName: val || null }));
        }
        appNameModal.classList.add("hidden");
      });

      appNameCancel.addEventListener("click", function () {
        appNameModal.classList.add("hidden");
      });

      appNameModal.querySelector(".confirm-backdrop").addEventListener("click", function () {
        appNameModal.classList.add("hidden");
      });
  ...
  function subscribePush() {
    navigator.serviceWorker.ready.then(function (reg) {
      ...
      navigator.serviceWorker.ready.then(function (reg) {
        var welcomeName = ($("footer-app-name") || {}).textContent || "Claude Relay";
        reg.showNotification("\ud83c\udf89 Welcome to " + welcomeName + "!", {
