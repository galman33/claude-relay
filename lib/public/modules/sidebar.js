...
function renderSessionItem(s) {
  ...
  var textHtml = "";
  if (s.worktree) {
    textHtml += '<span class="session-worktree-badge" title="Worktree: ' + s.worktree + '">' + iconHtml("git-branch") + '</span>';
  }
  if (s.isProcessing) {
...
export function initSidebar(_ctx) {
  ...
  // --- Worktree session modal ---
  var worktreeBtn = ctx.$("new-worktree-btn");
  var worktreeModal = ctx.$("worktree-modal");
  var worktreeInput = ctx.$("worktree-name-input");
  var worktreeOk = ctx.$("worktree-ok");
  var worktreeCancel = ctx.$("worktree-cancel");

  function openWorktreeModal() {
    worktreeModal.classList.remove("hidden");
    worktreeInput.value = "";
    setTimeout(function () { worktreeInput.focus(); }, 50);
  }

  function closeWorktreeModal() {
    worktreeModal.classList.add("hidden");
    worktreeInput.value = "";
  }

  function submitWorktree() {
    if (ctx.ws && ctx.connected) {
      var name = worktreeInput.value.trim();
      var msg = { type: "new_session", worktree: true };
      if (name) msg.worktreeName = name;
  ...
  if (worktreeOk) {
    worktreeOk.addEventListener("click", submitWorktree);
  }
  if (worktreeCancel) {
    worktreeCancel.addEventListener("click", closeWorktreeModal);
  }
  if (worktreeModal) {
    worktreeModal.querySelector(".confirm-backdrop").addEventListener("click", closeWorktreeModal);
  }
  if (worktreeInput) {
    worktreeInput.addEventListener("keydown", function (e) {
      if (e.key === "Enter") { e.preventDefault(); submitWorktree(); }
      if (e.key === "Escape") { e.preventDefault(); closeWorktreeModal(); }
    });
  }
