...
function createSDKBridge(opts) {
  ...
  function handleCanUseTool(session, toolName, input, opts) {
    ...
    var mode = session.mode || "ask";

    // Plan mode: deny mutating tools, auto-approve read-only tools
    if (mode === "plan") {
      if (MUTATING_TOOLS[toolName]) {
        return Promise.resolve({ behavior: "deny", message: "Blocked in plan mode — switch to ask or auto mode to make changes" });
      }
    ...
    // Ask mode (default): existing behavior below
