...
function createSDKBridge(opts) {
  ...
  var MUTATING_TOOLS = { Bash: 1, Edit: 1, Write: 1, NotebookEdit: 1 };

  function handleCanUseTool(session, toolName, input, opts) {
    // AskUserQuestion: always wait for user answers via WebSocket
    ...
    var mode = session.mode || "approve";

    // Plan mode: deny mutating tools, auto-approve read-only tools
    if (mode === "plan") {
      if (MUTATING_TOOLS[toolName]) {
        return Promise.resolve({ behavior: "deny", message: "Blocked in plan mode — switch to approve or auto mode to make changes" });
      }
    ...
    // Approve mode (default): existing behavior below
