 ...
 * opts: { tlsOptions, caPath, pinHash, port, debug, dangerouslySkipPermissions, appName }
 */
function createServer(opts) {
  ...
  var appName = opts.appName || "Claude Relay";

  var onAppNameChange = null;
  ...
  var pinPage = pinPageHtml(appName);
  ...
  var appHandler = function (req, res) {
    ...
    if (fullUrl === "/setup" && req.method === "GET") {
      ...
      res.end(setupPageHtml(setupUrl, setupUrl, !!caContent, lanMode, appName));
      return;
    }
    ...
    if (fullUrl === "/" && req.method === "GET") {
      ...
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
      res.end(dashboardPageHtml(statusList, currentVersion, appName));
      return;
    }
    ...
    // Dynamic manifest.json with configurable app name
    if (fullUrl === "/manifest.json" && req.method === "GET") {
      var manifest = {
        name: appName,
        short_name: appName,
        id: "/",
        ...
        icons: [
          ...
          { src: "icon-mono.svg", sizes: "any", type: "image/svg+xml" }
        ]
      };
    ...
    // Static files at root (favicon, icons, sw.js, etc.)
  ...
  if (tlsOptions) {
    onboardingServer = http.createServer(function (req, res) {
      ...
      if (url === "/setup" && req.method === "GET") {
        ...
        var lanMode = /[?&]mode=lan/.test(req.url);
        res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
        res.end(setupPageHtml(httpsSetupUrl, httpSetupUrl, !!caContent, lanMode, appName));
        return;
      }
  ...
  function addProject(cwd, slug, title) {
    ...
    var ctx = createProjectContext({
      ...
      onSetAppName: function (name) {
        if (onAppNameChange) onAppNameChange(name);
      },
    });
  ...
  function setAppName(name) {
    appName = name || "Claude Relay";
    pinPage = pinPageHtml(appName);
    // Update each project context so new connections get the right name
    projects.forEach(function (ctx) {
  ...
  return {
    ...
    set onAppNameChange(fn) { onAppNameChange = fn; },
