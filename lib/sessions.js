...
function createSessionManager(opts) {
  ...
  function saveSessionFile(session) {
    ...
    try {
      ...
      if (session.worktreeName) metaObj.worktreeName = session.worktreeName;
      if (session.worktreeCwd) metaObj.worktreeCwd = session.worktreeCwd;
  ...
  function loadSessions() {
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      var session = {
        ...
        worktreeName: m.worktreeName || null,
        worktreeCwd: m.worktreeCwd || null,
      };
  ...
  function createSession(sessionOpts) {
    ...
    var session = {
      ...
      history: [],
      messageUUIDs: [],
      worktreeName: sessionOpts.worktreeName || null,
      worktreeCwd: sessionOpts.worktreeCwd || null,
    };
    sessions.set(localId, session);
  ...
  function switchSession(localId) {
    ...
    send({ type: "session_switched", id: localId, cliSessionId: session.cliSessionId || null, worktree: session.worktreeName || null });
